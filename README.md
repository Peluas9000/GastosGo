#  GastosGo - Gestión de Gastos Personales [cite: 706, 707]
...
* **Usuario**: Almacena credenciales e identificadores[cite: 762, 769, 773].
* **Banco**: Gestiona el presupuesto o saldo mensual del titular[cite: 777, 778].
* **Categoria**: Clasificación de los gastos (ej. Alimentación, Transporte, Ocio)[cite: 798, 800].
* **Gasto**: Registro individual de cada transacción[cite: 806, 807].

**Relaciones de Integridad:**
* `Banco` → `Usuario`: Relación 1:1 con `CASCADE DELETE`[cite: 840].
* `Gasto` → `Usuario`: Relación N:1 con `CASCADE DELETE`[cite: 841].
* `Gasto` → `Categoria`: Relación N:1 con `SET NULL`[cite: 842].
...
* **Base de Datos Local**: Room Database y KSP (Kotlin Symbol Processing)[cite: 1370].
* **Asincronía**: Kotlin Coroutines (`lifecycleScope.launch`) para no bloquear la UI[cite: 1347, 1348, 1370].
* **Diseño UI**: Material Design y `MPAndroidChart` para gráficos[cite: 1370].
* **Gestión de Archivos**: Android `PdfDocument` y MediaStore API (Android 10+) para exportación segura de documentos[cite: 1364, 1370].

---

## 🚀 Posibles Mejoras Futuras

* Implementación de cifrado avanzado para contraseñas[cite: 1376].
* Exportación de datos masivos a CSV o Excel[cite: 1376].
* Filtros de búsqueda por fecha, categoría o rango de precios[cite: 1377].
* Sincronización en la nube y notificaciones push[cite: 1377].
* Implementación de Modo Oscuro nativo[cite: 1377].
